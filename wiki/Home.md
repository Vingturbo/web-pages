# 欢迎来到项目 Vingturbo/web-pages 的Wiki！
# Welcome to the Wiki for the Project Vingturbo/web-pages!

## 前言 & Perface

本项目是一个以纯 HTML 为核心的网页项目，致力于提供兼容性强、响应快速的访问体验。
This core language for this project is HTML, to view on many devices and the fast experience.

## 现状 & Now

我们正在寻找贡献者。
We are looking for contributors.

如果你是中文母语者，请进入[中文版文档](https://github.com/Vingturbo/web-pages/wiki/寻找贡献者.md)以查看完整招募内容。
If you are using English, please go into [The English Documentation](https://github.com/Vingturbo/web-pages/Looking_for_Contributor.md) to look the full text.
